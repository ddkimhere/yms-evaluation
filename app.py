...
if not selected_subjects:
    ...
    # 형태 유지를 위해 깔끔하게 이번달 입력창만 단일 칼럼 구조로 구현
        ...
        with st.spinner("AI가 학생의 점수 변화와 키워드를 분석하여 5문장 코멘트를 생성하고 있습니다..."):
            try:
                ...
                for idx, subj in enumerate(selected_subjects):
                    score_summary += f"- {subj}: 이번달 {current_scores[idx]}점\\n"
    ...
    if not selected_subjects:
        ...
        for i, subj in enumerate(selected_subjects):
            ...
            <tr style="border-bottom: 1px solid #f1f1f1;">
                ...
                <td style="padding: 12px; font-weight: bold; color: {LOGO_COLOR}; font-family: sans-serif; font-size:14px;">{current_scores[i]}점</td>
            </tr>
            """

        fig_width = max(5, len(selected_subjects) * 1.2)
        fig, ax = plt.subplots(figsize=(fig_width, 3.5))
        x_indices = range(len(selected_subjects))
        bar_width = 0.4
        
        # 이번 달 단일 막대그래프로 가독성 강화
        rects = ax.bar(x_indices, current_scores, bar_width, label='이번달 점수', color=LOGO_COLOR)
        ax.set_ylabel('점수 (점)')
        ax.set_title(f'{student_name} 학생의 영역별 성적 성취도', fontsize=12, fontweight='bold', pad=10, color=LOGO_COLOR)
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: center; font-size: 13px; margin-bottom: 30px; font-family: sans-serif; border: 1px solid #eaeaea;">
                    <thead>
                        <tr style="background-color: #f8f9fa; font-weight: bold; border-bottom: 1px solid #eaeaea; color:#555;">
                            <td style="padding: 12px; width: 50%;">평가 영역</td>
                            <td style="padding: 12px; width: 50%;">이번달 점수</td>
                        </tr>
                    </thead>
                    <tbody>{df_html_rows}</tbody>
                </table>
                <h3 style="font-size: 16px; color: {LOGO_COLOR} !important; margin-bottom: 15px; font-family: sans-serif; font-weight:700; border-left: 4px solid {LOGO_COLOR}; padding-left: 8px;">📊 영역별 성적 추이</h3>
